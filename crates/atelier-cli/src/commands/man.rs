    ...
    match role {
        ...
        Role::Manager => {
            if snapshot.and_then(|s| s.active_mission.as_ref()).is_some() {
                println!(
                    "  1. atelier issue status <id> - Review active mission readiness and blockers."
                );
                ...
                println!("  1. atelier issue table --kind mission - Choose a mission to inspect explicitly.");
