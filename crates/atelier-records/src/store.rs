...
impl RecordStore {
    ...
    pub fn allocate_issue_id(&self) -> Result<String> {
        allocate_record_id(|candidate| self.canonical_id_exists(candidate))
    }
...
fn allocate_record_id<F>(exists: F) -> Result<String>
where
    F: Fn(&str) -> Result<bool>,
{
    for _ in 0..MAX_ALLOC_ATTEMPTS {
        let id = format!(
            "{}-{}",
            record_id::DEFAULT_PROJECT_SLUG,
            random_base36_suffix(record_id::DEFAULT_SUFFIX_LEN)?
        );
        if !exists(&id)? {
            return Ok(id);
        }
...
fn random_base36_suffix(len: usize) -> Result<String> {
    ...
    Ok(record_id::base36_padded(value, len))
}
