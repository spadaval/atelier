...
use crate::models::{
    DomainRecord, EvidenceOutputSummary, EvidenceRecordData, Issue,
};
use crate::record_id;
...
fn render_evidence_record(
    ...
    write_yaml_scalar(&mut output, "captured_at", Some(&data.captured_at.to_rfc3339()))?;
...
fn parse_evidence_domain_record(
    ...
    let relationships = normalize_legacy_evidence_relationships(parse_relationships(
        &front_matter,
        relative,
    )?);
    let title = require_scalar(&front_matter, "title", relative)?;
...
fn normalized_domain_data_json(kind: &str, data_json: &str) -> Result<String> {
    ...
    if kind == "mission" {
        ...
        Ok(serde_json::to_string(&normalized_evidence_data(data_json)?)?)
    } else {
