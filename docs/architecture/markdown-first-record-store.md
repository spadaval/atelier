...
| `atelier issue link <id> <target> --role blocked_by` and `atelier issue unlink <id> <target> --role blocked_by` | RecordStore-owned Markdown-first | Typed issue relationship commands mutate canonical issue relationship front matter before projection refresh. Issue detail is the query surface and checks projection freshness. |
...
| Diagnostics, telemetry, tested marker, init, predecessor import, hidden renderer/projection probes, and check | Runtime, maintenance, import, or repair | Telemetry and work markers are local/runtime. The predecessor import bridge still renders canonical state from imported SQLite rows. Hidden renderer/projection probes are migration diagnostics, not normal durable mutation owners; `atelier check` and `atelier check --fix` own operator health and ignored-state repair. |
...
| `atelier history`, issue show recent activity, Agent Factory status helpers, hidden renderer/projection probes, and `atelier check` | Read-only consumers over `atelier-records::activity` | These surfaces may combine projection rows with sidecar events, but sidecar files remain the canonical history payload. |
| Predecessor-import preservation notes and close reasons | App/import workflow over `atelier-records::activity` | Imported predecessor comments are migration input and should be written as activity sidecars, preserving source timestamps when available. |
