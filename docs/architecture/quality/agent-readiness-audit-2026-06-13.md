...
| --- | --- | --- | --- |
| Committed onboarding entry points exist and point to durable repository sources. | pass | `AGENTS.md:5-36` requires Agent Factory skill usage, Atelier tracking, explicit issue commands, and durable `.atelier/` state. `AGENTS.md:28-49` names the docs map, product/architecture docs, and installed-binary rule. `docs/index.md:3-66` gives a committed documentation map. `atelier prime` printed the durable tracker state, ignored local state, essential commands, and repository notes. | none |
| Task discovery and work-selection paths are explicit and executable. | pass | This historical audit sampled command surfaces that have since been retired. Current normal paths are `atelier status`, `atelier work ready`, `atelier work mission <id>`, `atelier issue show <id>`, and `atelier issue transition <id>`, as defined by the current CLI surface. | none |
| Baseline handoff/health tooling is present and runnable. | pass | `AGENTS.md:47-66` names `git diff --check`, `cargo fmt -- --check`, `cargo nextest run`, `atelier export --check`, `atelier lint`, and `atelier doctor`. `rust-toolchain.toml:1-3` pins Rust 1.95.0 with `rustfmt` and `clippy`. `Cargo.toml:28-44` declares normal and dev dependencies for the Rust CLI. `cargo fmt -- --check` passed. `atelier doctor` passed and reported `projection_fresh: ok` and `rebuild_ready: ok`. | none |
