...
| Task discovery and work-selection paths are explicit and executable. | pass | `AGENTS.md:38-55` lists the common work/validation commands. `docs/product/cli-surface.md:43-78` and `:139-179` define `atelier prime`, `atelier status`, `atelier mission status`, and `atelier work queue --ready` as the normal operator surfaces. `atelier work queue --ready` returned a ready queue, and `atelier mission status atelier-man9` reported blockers, proof gaps, and next actions. | none |
