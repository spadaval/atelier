...
- Selective parallelism is the important lesson. Parallelize where coordination
  cost is low; keep sequential workflow validators where integration risk is
  high.
...
- Do not add direct agent lifecycle management in the current slice. The useful
  ...
  worktree state, workflow validators, and evidence.
...
- `atelier-v72a`: keep missions, milestones, plans, evidence, and workflow
  validators explicit enough to support milestone validation.
- `atelier-kitl`: workflow config should express validation phases, validator
  requirements, and lightweight versus strict flows.
- `atelier-9h7g`: Mission Control projection should emphasize scope, progress,
  blockers, branches/worktrees, evidence, validator failures, validation, and
  follow-up work.
