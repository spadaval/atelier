...
set derived from local issue statuses, active mission focus, ready work count,
tracker freshness, and the next
...
missions and issues, transition issue workflow status, manage blockers, record
...
| `status` | Root orientation for the current checkout. | Current `in_progress` work set, active mission, ready count, tracker freshness, and the next work/mission/health commands. | IDs, counts, and freshness token only. | `mission status`, `issue show <id>`, `issue list --ready`, `doctor`. |
...
| `start` | Current visible shortcut for moving an issue into the status-derived current work set. | Issue ID, resulting workflow status, mission/worktree context when available, and next proof or closeout commands. | Issue ID and status token only. | `issue transition <id> --options`, `status`, `issue show <id>`. |
| `abandon` | Current visible legacy pointer-cleanup surface while removal work is pending. | Explains that pause or handoff should be represented through issue workflow status, notes, and evidence; any remaining runtime pointer cleanup is temporary compatibility behavior. | Issue ID and cleared/unchanged token only. | `status`, `issue show <id>`, `issue transition <id> --options`. |
| `repair` | Current visible legacy stale-pointer recovery surface while removal work is pending. | Names any ignored runtime cleanup it performs and routes canonical or workflow problems to `lint`, `doctor`, and issue status inspection. | Issue ID or health token only. | `status`, `doctor`, `worktree status`, `issue show <id>`. |
...
- Select and run work: choose a clear slice, prepare the mission worktree and
  epic branch, move the issue to `in_progress`, leave notes, and advance or
  close it with proof. Owned by `atelier issue ...`, `atelier worktree ...`,
  `atelier branch ...`, record-specific note commands, and
  `atelier evidence ...`.
...
`atelier mission status` without an ID defaults to the configured mission focus
when one exists; otherwise it reports the available ready missions and the
command to select one. With an ID, the command is scoped to that mission
regardless of checkout focus. Default output is compact and answers: mission
...
- `active`: show current `in_progress` work, ready work, blockers, evidence gaps, and the next
...
unbounded logs.
The normal work lifecycle is status-derived. `atelier issue transition
<issue-id> start` moves an issue to `in_progress`; `atelier issue close
<issue-id> --reason "..."` completes accountable work after proof exists; other
workflow transitions, issue notes, blockers, or evidence records capture pause
and handoff context. These commands mutate canonical Markdown issue state and
do not create a hidden runtime active-work pointer. A separate durable claim
system is not part of the normal workflow unless a later assignment policy
justifies it.
The current work set is scoped to the checked-out tracker copy: every issue in
that `.atelier/` tree with status `in_progress` is current work for the
checkout. A checkout may therefore show multiple current issues, and another
worktree or branch may show a different set until the Markdown changes
reconcile through Git. Use the mission worktree for parallel work within the
same mission and the epic branch as the review boundary. Separate issue
worktrees are exceptional containment for conflicting, dirty, high-risk, or
explicitly assigned isolated slices.
...
| Root `start` active-pointer helper | Change/remove | Starting work is an issue workflow transition, not local runtime association creation. | `atelier issue transition <id> start`; if a root helper remains temporarily, it must perform only the same workflow transition and must not write `work_associations`. |
| Root `abandon` active-pointer cleanup | Remove | Local active-pointer cleanup is no longer a product concept once current work is status-derived. | Use workflow transitions, blockers, issue notes, and handoff evidence to pause or redirect work. |
| Root `repair` active-pointer cleanup | Remove | Stale active-work pointers should not exist in the target model. Runtime/cache repair belongs to `doctor --fix`; Git worktree cleanup belongs to `worktree status`, `worktree remove`, or ordinary Git recovery. |
...
| Hidden work-status helper and any legacy work-start path | Remove | Duplicate lifecycle paths obscure workflow-backed issue status and can recreate a runtime active pointer. | Docs and help teach `issue transition <id> start`, `issue close`, `status`, and `worktree`. |
...
- The normal `work start` and `work status` group; use
  `issue transition <id> start`, `issue close`, and `status`.
- Root `abandon` and root active-pointer `repair`; use issue workflow
  transitions, issue notes, blockers, evidence, `doctor --fix`, and worktree
  cleanup commands according to the actual state being changed.
...
Internal helpers may remain only when a core workflow still uses them. Legacy
session or `work_associations` rows must not decide current work; the current
work set comes from checked-out issue Markdown statuses.
...
| A Git worktree path is missing, dirty, or stale | `atelier worktree status` | Use `atelier worktree remove <id>` when the worktree record is intentionally obsolete, ordinary Git recovery when the checkout still matters, or `atelier doctor --fix` only for ignored runtime/cache/projection repair. |
| Current work set is surprising | `atelier status` then `atelier issue show <id>` for each listed issue | Inspect or change the issue workflow statuses in canonical Markdown; do not clear local runtime pointers. |
