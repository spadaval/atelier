...
| `start` | Shortcut for moving an issue into the status-derived current work set. | Issue ID, resulting workflow status, mission/worktree context when available, and next proof or closeout commands. It must not write runtime `work_associations` as current-work state. | Issue ID and status token only. | `issue transition <id> --options`, `status`, `issue show <id>`. |
...
| Root `start` convenience helper | Keep as thin workflow helper | Starting work is an issue workflow transition, not local runtime association creation. | `atelier start <id>` performs the same workflow transition as `atelier issue transition <id> start` and must not write `work_associations`. |
