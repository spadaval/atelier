...
| Inspect objective terminal readiness | `mission status`, `issue status`, `issue transition close --options` | `issue transition` plus summary in `issue show` | `atelier issue transition <objective-id> close` | Medium | Readiness gates belong to transition; show may summarize. |
| Show available workflow transitions | `issue transition --options`, `issue show` summary | `issue transition` | `atelier issue transition <id>` with no transition name | Medium | Long term, remove awkward `--options`. |
