...
| Complex commands need explicit budget verdicts. | Command-specific audit files record Keep/Simplify/Fold/Hide/Remove decisions. Evidence keeps typed reuse secondary to capture; history keeps only repository/issue scope plus a limit. Review and transition decisions remain with their owning work. | Enforce each recorded boundary in help, tests, and current product docs. |
