...
| --- | --- | --- | --- |
| Normal workflow | `status`, `issue show`, `issue transition --options`, `issue transition <id> start`, `issue transition <id> close --reason`, `mission status`, `evidence record`, `review status`, `lint` | `doctor`, `export`, `rebuild`, `workflow check`, `diagnostics slow`, `import-beads`, destructive `maintenance delete`, provider setup commands | Normal commands answer operator questions in domain terms and may be cited for ordinary handoff, validation, committed-state health, and terminal readiness. |
| Admin maintenance | `init`, `doctor`, `doctor --fix`, `prune`, `prune --apply`, `maintenance delete ... --force`, `forgejo roles check`, `forgejo roles provision`, `branch status`, `branch merge` | `mission status`, `issue transition --options`, hidden `workflow check`, hidden `diagnostics slow` | Admin commands configure or repair Atelier itself, recover owner branches manually, clean explicitly supported local artifacts, or perform explicit destructive record surgery. |
...
| `prune` | Keep visible admin | Keep dry-run by default. `--apply` may remove only cleanup classes with implemented retention contracts. | No new issue. |
| `bundle` | Keep visible manager/orchestrator | Keep as the reviewed bulk record creation surface. It replaces shell loops over issue/mission/evidence mutation commands, not normal single-record editing. | Rename implementation ownership away from `plan`. |
...
| `forgejo` | Keep visible admin/provider setup | Keep provider setup and permission checks out of normal role loops. Remove rejected legacy flags instead of preserving them. | Remove `--write-config`. |
| `graph` | Consolidate/remove visible namespace | Relationship and hierarchy questions should be answered from `issue show`, `mission show`, `mission status`, and blocker/evidence views. `graph tree` is duplicative; `graph impact` should not survive unless domain views still cannot answer blast-radius questions after they are strengthened. | Cutting target. |
...
| `worktree` | Remove/defer visible workspace management | Strip the root command pending redesign. Current work should be visible through issue/mission/status views; workspace isolation can use plain Git outside Atelier until a clearer product contract exists. | Tracked by `atelier-7eio`. |
