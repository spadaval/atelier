...
| --- | --- | --- | --- |
| Normal workflow | `status`, `issue show`, `issue transition --options`, `issue transition <id> <transition>`, `mission status`, `evidence record`, `lint`, `worktree for-mission` | removed root lifecycle opener/closer, `doctor`, `export`, `rebuild`, `workflow check`, `diagnostics slow`, `import-beads`, destructive `maintenance delete` | Normal commands answer operator questions in domain terms and may be cited for ordinary handoff, validation, committed-state health, and terminal readiness. Issue lifecycle changes run through workflow-owned transitions and declared effects. |
...
| `branch` | Keep visible advanced recovery | Keep for orchestrator recovery of epic owner branches. Routine branch setup and merge stay owned by issue transition effects. | Cross-reference: branch policy epic `atelier-5sjm`. |
