...
| --- | --- | --- | --- |
| `rebuild` | Hide/admin-frame | Keep as an advanced projection diagnostic. Normal local repair is `doctor --fix`; normal health proof is `lint` plus `doctor`. | Covered by `atelier-a7gd`; no new issue. |
