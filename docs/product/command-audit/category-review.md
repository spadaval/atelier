...
| `graph` | Removed visible namespace | Relationship and hierarchy questions are answered from `issue show`, `issue status`, `mission show`, `mission status`, and blocker/evidence views. `graph tree` was duplicative; `graph impact` moved into issue detail/status. | Removed by `atelier-39um`. |
