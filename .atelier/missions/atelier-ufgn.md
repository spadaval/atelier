...
id: "atelier-ufgn"
data: "{\"constraints\":[\"Do not implement during planning; keep existing .atelier-state compatibility read-only until an explicit migration slice lands.\",\"Canonical Markdown and runtime/cache concerns must remain separate in commands, docs, and tests.\"],\"evidence\":[],\"milestones\":[],\"plans\":[],\"risks\":[\"Persistence migration can corrupt durable tracker state if path resolution, ignore policy, and projection refresh are not centrally owned.\"],\"validation\":[\"Mission closeout requires workflow validation, lint, doctor, migration fixtures, direct-edit recovery tests, cache deletion recovery tests, cargo fmt -- --check, and cargo nextest run.\"],\"work\":[]}"
relationships:
  blocks: []
  children:
  - kind: "issue"
