...
- Mission is complete when docs, CLI help/man text, activity history, PR commands, status output, and tests all describe and enforce the new model.
- Validation must cover one-active-PR enforcement, PR issue inference, automatic worker/reviewer/validator attempt attribution, `session list/show` projections, and `pr merge` behavior.
- Final checks must pass: `cargo fmt -- --check`, `cargo nextest run`, `cargo nextest run --profile extended --run-ignored=only`, `atelier export --check`, `atelier lint`, and `atelier doctor`.
