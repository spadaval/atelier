...
schema_version: 1
status: recorded
title: "JSON removal targeted validation: cargo fmt -- --check, cargo test --test
  cli_integration -- --nocapture, git diff --check, atelier export --check, atelier
  lint, and atelier doctor passed. cargo nextest run had one unrelated/reproducible
  smoke concurrency failure: smoke::adversarial::test_concurrent_creates_5 reports
  Projection index is stale after concurrent creates."
