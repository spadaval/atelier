...
schema_version: 1
closed_at: '2026-06-19T21:42:11.757201411+00:00'
status: closed
...
- Manual check: Product docs and an ADR define transition effects, the review boundary,
  effect failure semantics, and the v1 effect set.
- Manual check: Workflow schema and planner tests accept declared transition effects, reject
  invalid effects, and expose a deterministic transition plan without executing
  it.
- Manual check: CLI tests show `atelier issue transition <id> --options`, blocked transition
...
- Manual check: Review integration tests prove the configured review artifact can be opened
  or reused from an explicit transition effect, while review commands still do
  not transition issue workflow.
- Manual check: Final validation evidence records focused tests, `atelier lint`,
...
Migrated from `.atelier/missions/atelier-grq8.md` as a declared mission objective issue.
