...
schema_version: 1
closed_at: '2026-06-15T04:35:03.342330031+00:00'
status: closed
...
- Manual check: Product docs and domain language define Mission = worktree, Epic = branch/review boundary, Issue = implementation slice.
- Manual check: Workflow policy and tests prove ordinary issues can close without review while epics require review/validation before done.
- Manual check: CLI commands create or locate a mission worktree and create/switch/status/merge epic branches without creating issue worktrees by default.
- Manual check: Agent Factory skill and repo binding instruct orchestrators to share mission worktrees, use epic branches for review, and avoid per-issue worktrees unless isolation is justified.
- Manual check: Migration evidence classifies existing issue worktrees and branches, removes clean contained worktrees, and preserves or folds unmerged/dirty state into the new model.
...
Migrated from `.atelier/missions/atelier-rxpr.md` as a declared mission objective issue.
