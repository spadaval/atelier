...
schema_version: 1
closed_at: '2026-06-10T22:18:35.594922100+00:00'
status: closed
...
- JSON output remains the automation contract and must stay stable unless separately approved.
- Color must improve scanning but cannot be the only carrier of meaning.
- Human output must work in narrow terminals and non-interactive logs.

### Risks

- Human-output changes may break users scraping text output; mitigate by documenting JSON as the stable contract and keeping text output explicitly human-oriented.
- Adding color and hierarchy inconsistently could make output noisier; mitigate with shared formatter primitives and snapshot tests.

## Evidence

- Manual check: Representative golden/snapshot coverage exists for mission show, issue show, issue list/ready/search, and compact hierarchy views across terminal width and color settings.
- Manual check: JSON snapshots or compatibility tests prove machine-readable output was not accidentally changed.
- Manual check: Docs describe the human-output patterns and when a command should use a table, sectioned detail view, grouped queue, or compact tree.

## Notes

Migrated from `.atelier/missions/atelier-g28t.md` as a declared mission objective issue.
