...
- Manual check: Committed product and architecture docs define retention classes, prune eligibility, safety defaults, and operator command shape.
- Manual check: Dry-run command output lists prune candidates with record kind, age or lifecycle reason, protection reason, and apply command.
- Manual check: Focused CLI tests reject active/open records, attached required proof, dirty worktrees, and unmerged branches, while allowing eligible stale local artifacts.
- Manual check: Final validation records command transcripts for focused prune tests, atelier lint, git diff --check, and documentation/help parity.

## Notes

Migrated from `.atelier/missions/atelier-24xn.md` as a declared mission objective issue.
